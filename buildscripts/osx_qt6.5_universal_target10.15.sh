#!/bin/bash -e
PROJECT=ImageViewer
BUILDDIR=build_osx_qt6.5_universal_target10.15
APPNAME="Image Viewer"
DMGNAME="${PROJECT}_qt6.5_universal_target10.15"
...
QT_PATH="${QT_PATH:=/opt/Qt/6.5.3/macos_target10.15}"
...
${CMD_QMAKE} -r CONFIG+="release" CONFIG+="hide_symbols" LIBS+=-dead_strip QMAKE_MAC_SDK=${MAC_SDK} QMAKE_MAC_SDK_VERSION=${MAC_SDK:6} QMAKE_MACOSX_DEPLOYMENT_TARGET=10.15 QMAKE_APPLE_DEVICE_ARCHS="x86_64 arm64" CONFIG+=c++2a CONFIG+="enable_update_checking" CONFIG+="enable_qtcore5compat" CONFIG+="system_resvg" INCLUDEPATH+="\"${RESVG_PATH}\"" LIBS+="-L\"${RESVG_PATH}\"" ${QMAKE_EXTRA_ARGS} "../${PROJECT}.pro"
make -j$(getconf _NPROCESSORS_ONLN)
cd "${OUT_PATH}"
plutil -replace LSMinimumSystemVersion -string "10.15" "${APPNAME}.app/Contents/Info.plist"
...
for unused_plugins_subdir in platforminputcontexts ; do
