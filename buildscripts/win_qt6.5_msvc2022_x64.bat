...
if "x%QT_PATH%x" == "xx" set QT_PATH=C:\Qt\6.5.3\msvc2019_64
set BUILDDIR=build_win_qt6.5_msvc2022_%ARCH%
set SUFFIX=_qt6.5_msvc2022_%ARCH%
...
mkdir %BUILDDIR%
cd %BUILDDIR%
qmake -r CONFIG+="release" CONFIG+="enable_update_checking" CONFIG+="system_resvg" INCLUDEPATH+=%RESVG_PATH% LIBS+=/LIBPATH:%RESVG_PATH% ..\%PROJECT%.pro
%NMAKE_CMD%
...
windeployqt --release --no-compiler-runtime --no-system-d3d-compiler --no-virtualkeyboard --no-pdf --no-opengl-sw --translations en,ru %PROJECT%%SUFFIX%
