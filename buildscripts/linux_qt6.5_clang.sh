...
mkdir -p "${BUILDDIR}"
cd "${BUILDDIR}"
${CMD_QMAKE} -r CONFIG+="release" CONFIG+=c++17 CONFIG+="enable_librsvg" CONFIG+="enable_update_checking" CONFIG+="system_resvg" INCLUDEPATH+="\"${RESVG_PATH}\"" LIBS+="-L\"${RESVG_PATH}\"" QMAKE_CFLAGS+="-Wno-implicit-function-declaration -Wno-incompatible-function-pointer-types" "../${PROJECT}.pro"
