...
cd "${BUILDDIR}"
BUILD_PATH="${PWD}"
arch -x86_64 ${CMD_QMAKE} -r CONFIG+="release" LIBS+=-dead_strip CONFIG+="x86_64" -r -spec unsupported/macx-clang QMAKE_MAC_SDK=${MAC_SDK} QMAKE_MACOSX_DEPLOYMENT_TARGET=${MAC_TARGET} CONFIG+="enable_macwebview enable_macwkwebview" "../${PROJECT}.pro"
