...
if "x%QT_PATH%x" == "xx" set QT_PATH=C:\Qt\6.5.3\msvc2022_arm64
...
mkdir %BUILDDIR%
cd %BUILDDIR%
qmake -r CONFIG+="release" CONFIG+="enable_update_checking" CONFIG+="enable_qtcore5compat" CONFIG+="system_resvg" INCLUDEPATH+=%RESVG_PATH% LIBS+=/LIBPATH:%RESVG_PATH% ..\%PROJECT%.pro
%NMAKE_CMD%
