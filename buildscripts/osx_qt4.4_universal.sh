...
cd "${SOURCE_PATH}"
${CMD_QMAKE} -macx -recursive -spec macx-xcode CONFIG+="release" LIBS+="-dead_strip" CONFIG+="x86 ppc" QMAKE_MAC_SDK="${MAC_SDK}" QMAKE_MACOSX_DEPLOYMENT_TARGET="${MAC_TARGET}" CONFIG+="disable_cxx11 disable_mactoolbar disable_mactouchbar" CONFIG+="enable_macwebview enable_nanosvg enable_j40"
