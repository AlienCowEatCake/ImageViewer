...
if "x%QT_PATH%x" == "xx" set QT_PATH=C:\Qt\5.15.12\msvc2022_arm64
...
set WEBVIEW2LOADER_DLL="%~dp0\..\src\ThirdParty\MSEdgeWebView2\microsoft.web.webview2.1.0.2535.41\build\native\%ARCH%\WebView2Loader.dll"
...
mkdir %BUILDDIR%
cd %BUILDDIR%
qmake -r CONFIG+="release" CONFIG+="disable_qtwebkit enable_update_checking" CONFIG+="enable_msedgewebview2 enable_mshtml enable_nanosvg" CONFIG+="system_resvg" INCLUDEPATH+=%RESVG_PATH% LIBS+=/LIBPATH:%RESVG_PATH% ..\%PROJECT%.pro
%NMAKE_CMD%
