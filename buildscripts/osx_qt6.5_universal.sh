...
${CMD_QMAKE} -r CONFIG+="release" CONFIG+="hide_symbols" LIBS+=-dead_strip QMAKE_MAC_SDK=${MAC_SDK} QMAKE_MAC_SDK_VERSION=${MAC_SDK:6} QMAKE_MACOSX_DEPLOYMENT_TARGET=${MAC_TARGET} QMAKE_APPLE_DEVICE_ARCHS="x86_64 arm64" CONFIG+=c++2a CONFIG+="enable_update_checking" CONFIG+="enable_qtcore5compat" CONFIG+="system_resvg" CONFIG+="disable_ghc_filesystem" INCLUDEPATH+="\"${RESVG_PATH}\"" LIBS+="-L\"${RESVG_PATH}\"" ${QMAKE_EXTRA_ARGS} "../${PROJECT}.pro"
