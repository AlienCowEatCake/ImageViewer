...
#pragma omp critical
    {
      ...
      uint32_t readbytes = input->read(&data[0], 1, toread*sizeof(uint64_t));
  ...
  if(readwords>0)
      invertBits(&data[0], readwords * sizeof(uint64_t));
  begin = newoffset;
                             ...
                             uint16_t left_margin)
{
  ...
  if (_extrahuff)
    big_huff_table = &extrahuff[0];

  uint16_t *gammatable = (gammaTable.size() >= 0x10000) && !noGammaFlag ? (uint16_t *)(&gammaTable[0]) : 0;
