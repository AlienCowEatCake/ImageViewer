...
#define P(NAME, MAIN_PRIORITY, ADVANCED_PRIORITY) decoderPriotities[QString::fromLatin1(NAME)] = ComplexPriotiry(MAIN_PRIORITY, ADVANCED_PRIORITY)
        ...
        P("DecoderReSVG"                , 2200, -1); ///< Умеет неинтерактивные svg, подгружает внешние библиотеки. Нестабилен, слишком строг к стандартам. Весьма медленная отрисовка.
        P("DecoderMacWebKit"            , 2300, -1); ///< Умеет неинтерактивные svg, только под macOS. Весьма медленная отрисовка.
        P("DecoderLibRSVG"              , 2400, -1); ///< Умеет неинтерактивные svg, подгружает внешние библиотеки. Весьма медленная отрисовка.
