...
public:
    ...
    PayloadWithMetaData<bool> readAvifFile(const QString &filePath)
    {
        ...
        for(m_numFrames = 0; ; m_numFrames++)
        {
            ...
            {
                LOG_DEBUG() << LOGGING_CTX << "Found PixelAspectRatioBox transform for frame" << m_numFrames;
                if(decoder->image->pasp.hSpacing != 1 && decoder->image->pasp.vSpacing != 1)
                {
                    LOG_DEBUG() << LOGGING_CTX << "pasp.hSpacing =" << decoder->image->pasp.hSpacing;
                    LOG_DEBUG() << LOGGING_CTX << "pasp.vSpacing =" << decoder->image->pasp.vSpacing;
                    /// @todo ?
                }
            }
            if(decoder->image->transformFlags & AVIF_TRANSFORM_CLAP)
            {
                LOG_DEBUG() << LOGGING_CTX << "Found CleanApertureBox transform for frame" << m_numFrames;
            ...
            {
                LOG_DEBUG() << LOGGING_CTX << "Found ImageRotation transform for frame" << m_numFrames;
...
#endif
            {
                LOG_DEBUG() << LOGGING_CTX << "Found ICC profile for frame" << m_numFrames;
