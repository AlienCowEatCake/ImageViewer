...
class DecoderKImageFormatsMovie : public IDecoder
{
public:
    QString name() const Q_DECL_OVERRIDE
    {
        return QString::fromLatin1("DecoderKImageFormatsMovie");
    }

    QStringList supportedFormats() const Q_DECL_OVERRIDE
    {
        const QList<QByteArray> readerFormats = KImageFormatsMovie::supportedFormats();
        QStringList result;
    ...
    QSharedPointer<IImageData> loadImage(const QString &filePath) Q_DECL_OVERRIDE
    {
        ...
        KImageFormatsMovie *movie = new KImageFormatsMovie(filePath);
        if(!movie->isValid() || movie->frameCount() == 1)
        {
            movie->deleteLater();
            return QSharedPointer<IImageData>();
        }
        IAnimationProvider *provider = new MovieAnimationProvider<KImageFormatsMovie>(movie);
...
DecoderAutoRegistrator registrator(new DecoderKImageFormatsMovie);
